...
public class square extends Drawing {
	...
	public void select() {
		...
		if (!selected) {
			...
			((Rectangle) bound).setWidth(((Rectangle) shape).getWidth());
			((Rectangle) bound).setX(((Rectangle) shape).getX());
			((Rectangle) bound).setY(((Rectangle) shape).getY());
			st = (Color) bound.getStroke();
	...
	public void keyboard_Edit() {
		...
		info.setSpacing(20);
		info.getChildren().addAll(minX, minY, width);
		ok.setOnAction(e -> {
			if (!check_parameter(minX.getText())) {
				...
				width.clear();
					...
					Double.parseDouble(width.getText()), Double.parseDouble(width.getText()))) {
	...
	public void deselect() {
		...
		if (selected) {
			...
			selected = false;
			bound.getStrokeDashArray().removeAll(3.0, 7.0, 3.0, 7.0);
